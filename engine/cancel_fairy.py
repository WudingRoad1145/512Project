...
class CancelFairy:
    ...
    async def cancel(self, order_msg, orderbooks): 
        ...
        self.logger.info(f"received cancel request for order {pretty_print_OrderRequest(order_msg)}")
        ...
        async with asyncio.Lock():
            if order_msg.order_id in self.active_orders.keys():
                if self.engine_addr != self.active_orders[order_msg.order_id]['address']:
                    self.logger.info(f"Routing cancel request to ME {self.active_orders[order_msg.order_id]['address']}")
                    response = await self.stubs[self.active_orders[order_msg.order_id]["address"]].CancelOrder(pb2.CancelOrderRequest(
                        order_id=order_msg.order_id,
                        client_id=order_msg.client_id,
                    ...
                    cancel_result = await orderbooks[order_msg.symbol].cancel_order(order_msg, self.active_orders, self.logger)
        ...
        async with asyncio.Lock():
            for fill in fills:
                ...
                if fill[1].order_id in self.active_orders.keys():
                    self.active_orders[fill[1].order_id]["remaining_quantity"] = fill[1].remaining_quantity

                    if self.active_orders[fill[1].order_id]["remaining_quantity"] <= 0:
                        try:
                            del self.active_orders[fill[1].order_id]
                        except Exception as e:
                            self.logger.warning(f"cancel key was not found on fills\nclient may have attempted to cancel a fully filled order \n Error message {e}")
